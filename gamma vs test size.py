...
for test_size in [.1,.3,.5,.7]:
    ...
    mae = {}
    cov = {}
    for gamma in [.01,.02,.1,.15,.2]:
        sim  = sim_matrix(M , 'Distance based' , gamma)
        predicts = predict(test_ratings , sim , train_rates , movies , users)
        mae[gamma]=MAE(predicts,actual)
        cov[gamma]=coverage(predicts)
    print('X: ',1-test_size)
    print('mae for different gammas: ',mae)
    print('coverage for different gammas: ',cov)
    print()
