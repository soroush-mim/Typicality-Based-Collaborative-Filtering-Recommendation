...
mae = {}
for sim_type in ['Distance based','Cosine based','Correlation based']:
    mae[sim_type]=[]
...
for test_size in [.1,.3,.5,.7]:
    ...
    for sim_type , gamma in [('Distance based',.01),('Cosine based',.96),('Correlation based',.8)]:
        
        sim  = sim_matrix(M , sim_type , gamma)
        predicts = predict(test_ratings , sim , train_rates , movies , users)
        mae[sim_type].append(MAE(predicts,actual))

plt.plot([0.9,.7,.5,.3] , mae['Distance based'] , label ='Distance based' )
plt.plot([0.9,.7,.5,.3] , mae['Cosine based'], label ='Cosine based' )
plt.plot([0.9,.7,.5,.3] , mae['Correlation based'], label ='Correlation based' )
plt.legend()
